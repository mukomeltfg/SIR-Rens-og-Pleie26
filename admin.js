...
function showSetup(message='Supabase er ikke konfigurert ennå. Fyll inn Project URL og anon key i config.js.'){
  ...
  if($('#loginPanel'))$('#loginPanel').hidden=true;if($('#adminPanel'))$('#adminPanel').hidden=true;if($('#logoutBtn'))$('#logoutBtn').hidden=true;
}
...
async function showState(){
  ...
  try{const s=await session();const allowed=s?.user?.email?.toLowerCase()===String(cfg.adminEmail||'').toLowerCase();$('#loginPanel').hidden=!!allowed;$('#adminPanel').hidden=!allowed;$('#logoutBtn').hidden=!allowed;$('#setupWarning').hidden=true;if(allowed){await loadOrders();await loadSettings()}else if(s)await db.auth.signOut()}
...
document.querySelectorAll('.admin-tab').forEach(btn=>btn.onclick=()=>{
  ...
  document.querySelectorAll('.admin-tab-panel').forEach(p=>p.hidden=p.id!==`tab-${btn.dataset.tab}`);
});

$('#loginForm').onsubmit=async e=>{e.preventDefault();const status=$('#loginStatus');if(!db?.auth&&!initSupabase()){status.textContent='Supabase er ikke konfigurert.';return}status.textContent='Вход…';const email=$('#adminEmail').value.trim();if(email.toLowerCase()!==String(cfg.adminEmail||'').toLowerCase()){status.textContent='Эта почта не имеет доступа к управлению.';return}const {error}=await db.auth.signInWithPassword({email,password:$('#adminPassword').value});status.textContent=error?error.message:'';if(!error)showState()};
...
const defaults={primary_color:'#c99a4b',background_color:'#07111f',card_color:'#101d2d',hero_title:'Renere interiør. Friskere hjem. Bedre inntrykk.',hero_text:'Profesjonell dyprens av bilinteriør og tekstilmøbler. Send bilder og få en individuell vurdering.',promo_text:'',promo_enabled:false,promo_code:'SIR10'};
async function loadSettings(){
  ...
  $('#primaryColor').value=v.primary_color;$('#backgroundColor').value=v.background_color;$('#cardColor').value=v.card_color;
  $('#heroTitleSetting').value=v.hero_title||'';$('#heroTextSetting').value=v.hero_text||'';$('#promoTextSetting').value=v.promo_text||'';
  $('#promoEnabled').value=String(Boolean(v.promo_enabled));$('#promoCodeSetting').value=v.promo_code||'SIR10';
}
$('#settingsForm').onsubmit=async e=>{
  e.preventDefault();const status=$('#settingsStatus');status.textContent='Lagrer…';
  const payload={id:1,primary_color:$('#primaryColor').value,background_color:$('#backgroundColor').value,card_color:$('#cardColor').value,hero_title:$('#heroTitleSetting').value.trim(),hero_text:$('#heroTextSetting').value.trim(),promo_text:$('#promoTextSetting').value.trim(),promo_enabled:$('#promoEnabled').value==='true',promo_code:$('#promoCodeSetting').value.trim()||'SIR10',updated_at:new Date().toISOString()};
  const {error}=await db.from('site_settings').upsert(payload,{onConflict:'id'});status.textContent=error?error.message:'Lagret. Endringene vises på nettstedet.';if(!error)toast('Nettstedet er oppdatert');
};
